...
##instanceListNm_frt = "/home/ansible/pythonTools/data/instanceList.txt_before"
...
if initcnt > 0:
  ...
  # inituser作成コマンドの発行
  cmd = "ansible-playbook -i " + init_inventry + " --user ec2-user " + playbookdir + "inituser/addInituser.yml"
  proc = subprocess.call( cmd , shell=True)
  ...
  fhn.write("ansible_ssh_private_key_file=/home/ansible/.ssh/inituser_id_rsa\n")
  fhn.close()

  # zabbix-agentインストールコマンドの発行
  cmd = "ansible-playbook -i" + za_invent + " --user inituser " + playbookdir + "zabbix/zabbixAgent_install.yml"
  proc = subprocess.call( cmd , shell=True)
  ...
  for tmpInitIP in allInitList:
    ...
    for insinfo in allInstanceList:
      if insinfo.find(thisip) >= 0:
        ...
        cmd = "aws ec2 create-tags --resources " + targetid + " --tags Key=Name,Value=ready"
        proc = subprocess.call( cmd , shell=True)
...
if os.path.isfile(za_invent):
  os.remove(za_invent)
##if os.path.isfile(tmpInitlist):
##  os.remove(tmpInitlist)
